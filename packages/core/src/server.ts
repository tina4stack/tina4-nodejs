...
export function resolveTemplate(pathname: string, templatesDir: string): string | null {
  ...
  if (isDev) {
    ...
    // ADR-0078: a "." / ".." segment (or a backslash) could walk out of pages/
...
async function runMatchedRoute(ctx: MatchedRouteContext): Promise<void> {
  ...
  // DEVADMIN-DEC-01/02 + ADR-0078: fail-closed gate BEFORE the handler runs.
  // Closes drive-by CSRF, cross-site reads, DNS rebinding and a network-exposed
  // debug box. Scoped to /__dev so /__feedback + /ai are unaffected. The MCP
  // endpoints keep their own 404 gate (devRequestDenial skips the mcp prefixes
  // for the loopback part).
  // ADR-0078: ONE gate for every /__dev request, reads included - Host
  // allow-list (DNS rebinding), same-origin, loopback peer.
  if (ctx.pathname.startsWith("/__dev")) {
...
export async function startServer(config?: Tina4Config): Promise<{
  ...
  server.on("upgrade", (req: IncomingMessage, socket: Socket, head: Buffer) => {
    ...
    if (isDevMode() && upPath === "/__dev_reload") {
      // ADR-0078: the reload socket answers only a loopback / TINA4_HOST Host.
