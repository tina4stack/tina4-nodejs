...
export class DevAdmin {
  ...
  /**
   * Returns the overlay script to inject into HTML pages.
   */
  static renderOverlayScript(): string {
    return renderOverlayScript();
  }
}
...
function renderOverlayScript(): string {
  return `<script>
(function(){
  ...
  btn.innerHTML = 'T4';
  btn.title = 'Tina4 Dev Dashboard';
  btn.style.cssText = 'position:fixed;bottom:1rem;right:1rem;width:2.5rem;height:2.5rem;background:#2e7d32;color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-weight:700;font-size:0.8rem;font-family:system-ui;z-index:99999;box-shadow:0 2px 8px rgba(0,0,0,0.3);transition:transform 0.15s,opacity 0.15s;opacity:0.5';
