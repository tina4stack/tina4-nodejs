...
*/
/**
 * Dev-surface gate contract (dev-surface security regression) — REAL server, REAL sockets, NO mocks.
 *
 * Exercises privileged dev endpoint access through
...
const BASE_DIR = mkdtempSync(join(tmpdir(), "tina4-dev-surface-"));
const LAN_IP = Object.values(networkInterfaces()).flat().find((i) => i && !i.internal && i.family === "IPv4")?.address;
...
function request(method: string, port: number, path: string, body: unknown, headers: Record<string, string> = {}):
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({
      host: currentPeer, port, path, method, agent: false,
      headers: {
        ...(payload !== undefined ? { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(payload) } : {}),
        Connection: "close", Host: "localhost", ...headers,
      },
    }, (res) => {
      let raw = "";
      res.on("data", (c) => (raw += c));
...
console.log("=== Dev-surface gate contract (dev-surface security regression) — real server, no mocks ===\n");
...
setEnv({ TINA4_DEBUG: "false" });
{
  ...
  assert("health retains the release version outside debug",
    health === "200:framework,status,uptime,version" && underscored === "200:framework,status,uptime,version",
...
await new Promise<void>((r) => peerServer.listen(0, "0.0.0.0", () => r()));
...
{
  symlinkSync(join(SIBLING_DIR, "missing.txt"), join(TEST_DIR, "dangling.txt"));
  const denied = await post(port, "/__dev/api/file/save", { path: "dangling.txt", content: "x" });
  assert("a dangling symlink cannot write outside the project", denied.status === 403 && !existsSync(join(SIBLING_DIR, "missing.txt")));
}
...
}
{
  ...
  for (const method of ['GET', 'HEAD', 'OPTIONS', 'POST', 'PUT', 'PATCH', 'DELETE']) {
    const r = await request(method, port, "/__dev/api/file?path=readme.txt", undefined);
    assert(`every dev method requires raw peer trust: ${method}`, r.status === 403 && !r.raw.includes("public-readme"));
  }
  currentPeer = "127.0.0.1";
}

// Actual origins include scheme and port; fetch metadata cannot override Origin.
{
  const host = `localhost:${port}`;
  for (const origin of [`http://localhost:${port + 1}`, `https://localhost:${port}`, "null", "http://sibling.example"]) {
    const r = await get(port, "/__dev/api/file?path=readme.txt", { Host: host, Origin: origin, "Sec-Fetch-Site": "same-origin" });
    assert("conflicting origin is refused", r.status === 403);
  }
  const allowed = await get(port, "/__dev/api/file?path=readme.txt", { Host: host, Origin: `http://${host}` });
  assert("matching scheme host and port is allowed", allowed.status === 200 && allowed.raw.includes("public-readme"));
  currentPeer = LAN_IP!;
  const options = await request("OPTIONS", port, "/__dev/api/file?path=readme.txt", undefined);
  assert("a non loopback peer cannot request OPTIONS", options.status === 403);
  const head = await request("HEAD", port, "/__dev/api/file?path=readme.txt", undefined);
  currentPeer = "127.0.0.1";
  assert("a non loopback peer cannot read HEAD", head.status === 403);
}
...
// ── only TINA4_MCP_TOKEN unlocks the remote dev surface ─────────────────────
{
  ...
  const wrongHost = await post(port, "/__dev/api/file/save", { path: "token_host_probe.txt", content: "x" },
    { Authorization: "Bearer mcp-token-0078", Host: "rebind.evil.example" });
  const wrongOrigin = await post(port, "/__dev/api/file/save", { path: "token_origin_probe.txt", content: "x" },
    { Authorization: "Bearer mcp-token-0078", Origin: "http://foreign.example" });
  assert("dedicated token does not bypass host or origin", wrongHost.status === 403 && wrongOrigin.status === 403
    && !existsSync(join(TEST_DIR, "token_host_probe.txt")) && !existsSync(join(TEST_DIR, "token_origin_probe.txt")));
  const ok = await post(port, "/__dev/api/file/save", { path: "api_key_probe.txt", content: "ok" },
    { Authorization: "Bearer mcp-token-0078", Host: "localhost:7148" });
  currentPeer = "127.0.0.1";
