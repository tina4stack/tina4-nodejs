...
await withEchoServer(async (hit) => {
  const url = await hit("evil.com");
  assert(
    "untrusted peer: X-Forwarded-Host is ignored for request.url",
...
await withEchoServer(async (hit) => {
  const url = await hit("app.example.com");
  assert(
    "trusted peer: X-Forwarded-Host is honoured for request.url",
