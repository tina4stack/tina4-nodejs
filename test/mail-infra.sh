...
# (test/mailTransportTls.test.ts; vendored byte-for-byte in behaviour from
...
# DNS:localhost and IP:127.0.0.1. The tests trust that CA through NODE_EXTRA_CA_CERTS,
...
#
# Usage:  bash test/mail-infra.sh [dir] [name-prefix]
...
#         `bash test/mail-infra.sh [dir] [name-prefix] down` removes them.
set -e
...
# ---------------------------------------------------------------- TLS material
# Same recipe as test/mqtt-infra.sh: the CA needs basicConstraints AND
...
# account above are fixed by this script, and test/mailTransportTls.test.ts uses
